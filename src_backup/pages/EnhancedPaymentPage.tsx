...
export default function EnhancedPaymentPage() {
  ...
  const location = useLocation() as { state?: { repay?: boolean; paymentId?: string; amount?: number; method?: string } };
  ...
  const isRepay = Boolean(location?.state?.repay);
  ...
  // Prefill from repayment context (amount/method) as soon as available
  useEffect(() => {
    if (isRepay) {
      if (typeof location?.state?.amount === 'number') {
        setReportAmount(location.state.amount);
      }
      if (location?.state?.method) {
        setSelectedMethod(location.state.method);
      }
    }
  }, [isRepay, location?.state]);
    ...
    try {
      ...
      const { error: paymentError } = await supabase
        ...
        .insert({
          ...
          proof_url: proofUrl,
          status: 'pending',
          phonepe_transaction_id: selectedMethod === 'razorpay' ? razorpayPaymentId : null
        });
      ...
      toast({
        title: isRepay ? "Re-payment Submitted" : "Payment Submitted",
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        >
          <h1 className="text-2xl sm:text-3xl font-bold gradient-text mb-2">
            {isRepay ? 'Re-submit Payment for Rejected Request' : 'Complete Payment 💳'}
          </h1>
          <p className="text-muted-foreground text-sm sm:text-base">
            {isRepay ? 'Please re-submit your payment with updated proof.' : 'Complete payment for your approved report'}
          </p>
        </motion.div>
            ...
            <GlassCard className="p-6">
              ...
              {selectedMethod === 'offline' && (
                <div className="space-y-4">
                  ...
                  {/* Transaction ID / Reference Number removed as requested */}
