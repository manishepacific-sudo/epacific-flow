  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const approvedThisMonth = reportsData.filter((report: Report) => {
        const reportDate = new Date(report.approved_at || report.created_at);
        return (
      ...
      setStats({
        ...
        pendingReports: reportsData.filter((r: Report) => r.status === "pending_approval").length,
      ...
      <div className={cn("space-y-6", isMobile && "pb-20")}>
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold gradient-text">{(() => { const h=new Date().getHours(); return h<12?'Good morning':h<17?'Good afternoon':h<21?'Good evening':'Good night'; })()}, {profile?.full_name?.split(' ')[0] || 'Manager'}!</h1>
              <p className="text-muted-foreground">
                Manager Dashboard — Overview of system activity and pending tasks
              </p>
            </div>
            <Button
              variant="outline"
                          ...
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              {(() => { const props = getReportBadgeProps(report.status); return (
                                <Badge variant={props.variant} className={`mt-1 ${props.className}`}>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end">
                                <DropdownMenuItem onClick={() => handleViewReportDetails(report)} className="text-foreground">
                        ...
                        <div className="flex items-center justify-between gap-4">
                          <div className="flex items-center gap-3 flex-1 min-w-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <Badge className={`mt-1 ${getPaymentBadgeClass(payment.status)}`}>
                  ...
                  <TableBody>
                    {payments.map((payment) => (
                      <TableRow key={payment.id}>
                        ...
                        <TableCell>
                          <Badge className={getPaymentBadgeClass(payment.status)}>
                            {getPaymentStatusLabel(payment.status)}
                          </Badge>
                        </TableCell>
                        <TableCell>
