...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=User Creation and Invitation Successful').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Admins and managers could not create and invite new users with appropriate role assignments, or invited users did not receive notifications as expected.')
