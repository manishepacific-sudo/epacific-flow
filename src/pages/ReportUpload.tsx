...
import { getReportBadgeProps, getReportStatusLabel } from "@/utils/statusBadges";
import { Calendar } from "@/components/ui/calendar";
import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";
import { DataPreview } from "@/components/ui/data-preview";
import { cn } from "@/lib/utils";
import { notifyReportUploaded } from "@/utils/notifications";
...
export default function ReportUpload() {
  ...
  const [reportDatePopoverOpen, setReportDatePopoverOpen] = useState<boolean>(false);
  ...
  const location = useLocation() as { state?: { resubmit?: boolean; reportId?: string; skipPaymentIfPaymentApproved?: boolean; previousData?: { title?: string; description?: string; amount?: number } } };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (uploadError) {
        ...
        throw new Error(uploadError.message || 'Failed to upload report file');
      }
      ...
      const basePayload = {
        ...
        description: 'Report submitted via file upload',
      ...
      let reportInsertError: any | null = null;
      let report: any | null = null;

      // First try with report_date (newer schema)
      {
        const { data, error } = await supabase
          .from('reports')
      ...
      if (reportInsertError) {
        ...
        throw new Error(reportInsertError.message || 'Failed to create report record');
      }

      // Send notification to managers
      await notifyReportUploaded(user.id, report.title || 'Monthly Report');

      const comingFromResubmit = Boolean(location?.state?.resubmit);
      const skipPayment = Boolean(location?.state?.skipPaymentIfPaymentApproved);

      if (comingFromResubmit && skipPayment) {
        toast({
          title: "Report resubmitted",
          description: "No payment required as payment is already approved.",
        });
        navigate(`/report/${report.id}`);
      } else {
        toast({
          title: comingFromResubmit ? "Report resubmitted" : "Report sent for approval",
      ...
      const hint = typeof message === 'string' && message.toLowerCase().includes('row-level')
        ? 'You may not have permission to submit reports. Please contact support.'
        : undefined;
      toast({
            ...
            <GlassCard>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Popover open={reportDatePopoverOpen} onOpenChange={setReportDatePopoverOpen}>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        onSelect={(date) => {
                          if (!date) return;
                          setReportDate(date);
                          setReportDatePopoverOpen(false);
                        }}
                        initialFocus
              ...
              <div className="space-y-3">
                {reportHistory.map((report) => (
                  <div
                    ...
                    className="p-4 rounded-lg border border-border hover:bg-accent/50 transition-colors cursor-pointer focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <h4 className="font-medium truncate">{report.title}</h4>
                          {(() => { const props = getReportBadgeProps(report.status); return (
