...
export default function AttendanceManagementPage() {
  ...
  const handleAttendanceAction = async (
    ...
    try {
      ...
      const { error } = await supabase.functions.invoke('update-attendance-status', {
        body: { attendanceId, action, approverUserId: profile?.user_id, remarks: notes }
      });
    ...
    <Layout role={profile?.role}>
      <div className={cn("space-y-6", isMobile && "pb-20")}>
        ...
        <SearchFilterExport
          ...
          filters={{ ...filters, useReportDate: false }}
